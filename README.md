...
### Step 1.1 — Replace social preview assets

Before deployment, always replace these starter assets:

- `public/og-image.jpg` — 1200x630 or larger brand-specific image for Telegram, VK, Twitter/X, and other link previews.
- `public/favicon.svg` and `public/favicon.ico` — brand favicon.

Set the production URL in `.env` or in your hosting provider:

```bash
PUBLIC_SITE_URL=https://your-production-domain.ru
```

`PUBLIC_SITE_URL` must match the real public domain. `BaseLayout.astro` converts `seo.ogImage` to an absolute URL, because Telegram and other messengers cache previews aggressively and may keep showing an old project's image when the URL or asset is reused.

After changing the preview image on an already shared link, force a fresh fetch in Telegram by sending the URL to `@WebpageBot` and choosing "Update preview". If the domain changed, update `public/robots.txt` so its `Sitemap:` line points to the same production domain.
